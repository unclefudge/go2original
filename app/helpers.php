...
define("REQUIRED_FIELD", '<span class="m--font-danger">*</span>');
...
{
    if ($errors->has($field))
        return '<span class="form-control-feedback">' . $errors->first($field) . '</span>';
}

function fieldHasError($field, $errors)
{
    if ($errors->has($field))
        return 'has-danger m--font-danger m--font-bolder';
}
